...
export default async function StatusPage({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/70 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            <a href="/" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
              ← Dashboard
            </a>
            <a href={m?.url} target="_blank" rel="noreferrer" className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow hover:bg-slate-800">
              Visit URL
            </a>
      ...
      {/* Content (unchanged from your styled version) */}
      <main className="mx-auto max-w-6xl px-6 py-8">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight">{m.name}</h1>
              <a href={m.url} target="_blank" rel="noreferrer" className="mt-1 block max-w-full truncate text-sm text-slate-600 underline-offset-2 hover:underline" title={m.url}>
                {m.url}
              </a>
            ...
            <a href={`/`} className="hidden rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 sm:inline-flex">
              Back
            </a>
          </div>
        ...
        <section className="mt-6 flex flex-wrap items-center gap-3">
          <a href={`/status/${slug}`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" title="Refresh">
            Refresh
          </a>
          <a href={`/`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
            View all monitors
          </a>
          <span aria-disabled="true" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-400 cursor-not-allowed select-none" title="Public JSON (coming soon)">
            Public JSON
          </span>
        </section>
            ...
            <dl className="mt-3 space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <dd className="truncate font-medium text-slate-900" title={m.url}>
                  <a className="underline-offset-2 hover:underline" href={m.url} target="_blank" rel="noreferrer">
                    {m.url}
                  </a>
                </dd>
              </div>
